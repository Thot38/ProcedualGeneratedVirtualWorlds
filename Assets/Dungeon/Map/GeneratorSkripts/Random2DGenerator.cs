...
public class Random2DGenerator : IGenerator
{
    ...
    public Random2DGenerator(float increment, int numberOfAdjacentsIn2D, int numberOfSmoothings, float landMasses, string seed)
    {
    ...
    public void Fill(ref float[,,] map, int width, int height, int depth)
    {
        ...
        {
            Smooth2D(ref map, width, height, depth);
        }
    }

    private void Smooth2D(ref float[,,] map, int width, int height, int depth)
    {
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
